...
DELIMITER $$
CREATE TRIGGER InserirDataDeAtualizacao
...
USE betrybe_automoveis;
DELIMITER $$
CREATE TRIGGER AtualizarDataDeAtualizacao
...
-- Além disso, crie um outro Trigger para INSERT que adiciona um novo registro na tabela movies_logs, informando o movie_id do filme
-- que acaba de ser inserido na tabela movies, a action como 'INSERT' e a log_date como a data atual.
USE BeeMovies;
DELIMITER $$ 
CREATE TRIGGER AtualizarTabelaMoviesLogs
...
USE BeeMovies;
DELIMITER $$
CREATE TRIGGER AtualizarFilmes
...
-- Exercício 3: Crie um Trigger na tabela movies que, ao ter algum de seus registros excluídos, 
-- deve enviar uma informação para a tabela movies_logs, 
-- onde deve ser guardada a data da exclusão, a action 'DELETE' e o id do filme excluído.
USE BeeMovies;
DELIMITER $$
CREATE TRIGGER AtualizarDataDeAtualizacao
